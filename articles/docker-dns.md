...
The only requirements for the examples that follow are [Vagrant](https://www.vagrantup.com/) and [Git](https://git-scm.com/). I choose Vagrant over [Docker Machine](https://www.docker.com/products/docker-machine) so that everyone can use the same commands no matter whether host OS is Linux, OS X, or Windows. The principles explained here are the same no matter whether your setup. Please make sure that both are installed on your machine. Everything else will be provisioned automatically.
...
Let's see the networking in action. We'll run a simple service consisting of two containers. They are defined as targets `app` and `db` inside the [docker-compose-internal.yml](https://github.com/vfarcic/docker-flow/blob/master/articles/dns/docker-compose-internal.yml) file. Its content is as follows.
...
As you can see, besides networks created during the Swarm setup, the *backend* network is available as well. We can use it through [docker-compose.yml](https://github.com/vfarcic/docker-flow/blob/master/articles/dns/docker-compose.yml). The content of the file is as follows.
...
![DNS load balancing through aliases](img/good-the-bad-and-the-ugly)
