...
type Opts struct {
	...
	ComposePath             []string `short:"f" long:"compose-path" value-name:"docker-compose.yml" description:"Path to the Docker Compose configuration file." yaml:"compose_paths" envconfig:"compose_path"`
	ComposePathOld          string   `yaml:"compose_path"` // Added for compatibility with the old format. TODO: Remove
...
var GetOpts = func() (Opts, error) {
	opts := Opts{
		ComposePath: []string{dockerComposePath},
		Flow:        []string{"deploy"},
	}
...
func ParseEnvVars(opts *Opts) error {
	...
	data := []struct {
		...
		{"FLOW_COMPOSE_PATH", &opts.ComposePath},
	}
	for _, d := range data {
...
func ProcessOpts(opts *Opts) (err error) {
	...
	if len(opts.ComposePath) == 0 && len(opts.ComposePathOld) > 0 {
		opts.ComposePath = []string{opts.ComposePathOld}
	}
